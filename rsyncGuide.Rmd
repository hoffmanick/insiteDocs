...
This guide is intended to provide some minimum information on the software tool rsync for INSITE's data producers, so they can send large files remotely to our file server. This guide is a quick start. If you would like more information about advanced rsync use, please consult this <a href="https://linux.die.net/man/1/rsync" target="_blank">page</a>.
...
rsync commands must be input into a command-line interface (CLI) like Terminal or Command Prompt. rsync involves syncing files between two different computer systems, which must both be rsync-compatible in order for the operation to succeed. The INSITE server is rsync-compatible, and if the computer from which you're transferring files uses a Linx or Mac OS, it should be rsync-compatible too. If you are using a Windows operating system, see below for more information.  
...
To send a file from a local computer to a remote one, the following syntax is appropriate:

```{r rsync-syntax, include=TRUE,echo=FALSE}
noquote("rsync [optionalFlags] [pathToLocalFile] [pathToRemoteFolder]")
```

In our case, the remote folder to which we will be sending our files is the INSITE server, so we can replace *[pathToRemoteFolder]* with *[Username]@[ServerIPAddress]::[folder]*. The server IP address is 169.236.184.28, and our shared folder for INSITE data is insiteData/data/... . Your username is specific to you. The rsync syntax may then look like:

```{r rsync-syntax2, include=TRUE,echo=FALSE}
noquote("rsync [optionalFlags] [pathToLocalFile] NickHoffman@169.236.184.28::insiteData/data/leafhopper/theme2/genome/exp5/")
```

If you're performing your rsync from within the folder in which reside the data you intend to send, then you simply need to replace *[pathToLocalFile]* with the name of your file. If you are performing rsync from a distinct folder, you need to input the path from the folder you're currently in to the location of your file. For the sake of simplicity, let's assume we're sending from the folder in which our data reside, and the name of our genomic leafhopper data file is Hoffman_09272024_3.fastq. In that case, you may perform the following command, omitting for now the [optionalFlags]: 

```{r rsync-syntax3, include=TRUE,echo=FALSE}
noquote("rsync Hoffman_09272024_3.fastq NickHoffman@169.236.184.28::insiteData/data/leafhopper/theme2/genome/exp5/")
```
...
noquote("rsync -avz Hoffman_09272024_3.fastq NickHoffman@169.236.184.28::insiteData/data/leafhopper/theme2/genome/exp5/")
```

You are of course encouraged to explore other flags linked above.
...
noquote("rsync  version x.x.x  protocol version xxx")
```
