...
This guide will provide instructions on writing a README guide for data produced by INSITE-affiliated researchers.

# When do I write a README?

If you are producing any raw data with or without downstream products (final datasets, manuscripts), you should be uploading those data and any downstream products to the INSITE server. As well, you should be writing a README that provides a rich description of what those data are.
...
And you would name write a README with the following name:
...
Repositories of sequencing data like GenBank have specific metadata requirements for upload. If you are uploading sequencing data to the INSITE server, you must find the <a target="_blank" href="https://ncbi.nlm.nih.gov/biosample/docs/packages/">relevant metadata specification</a> from NCBI and ensure that you have included all mandatory metadata fields from that list.

For instance, for <a href="https://www.ncbi.nlm.nih.gov/biosample/docs/packages/MIMARKS.specimen.host-associated.5.0/" target="_blank">marker gene sequence from a specimen associated with a host</a>, NCBI requires:
...
These metadata values <b>must</b> be findable in your READMEE file. The definitions of these metadata fields can be found on <a href="https://ncbi.nlm.nih.gov/biosample/docs/packages/MIMARKS.specimen.host-associated.6.0/" target="_blank">the NCBI MIMARKS page</a>.
